...
export function renderCallbackPage(params: CallbackPageParams): string {
  ...
  const successPayload = JSON.stringify({
    type: 'soundcloud-desktop-auth-success',
    sessionId: sessionId || '',
  });
  const errorPayload = JSON.stringify({
    type: 'soundcloud-desktop-auth-error',
    error: error || '',
  });
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    body::before {
      ...
      height: 200%;
      background:
        radial-gradient(circle at 30% 40%, rgba(255, 85, 0, 0.08) 0%, transparent 50%),
...
<body>
  ...
  <script>
    try {
      const payload = ${success ? successPayload : errorPayload};
      if (payload.type === 'soundcloud-desktop-auth-success' && payload.sessionId) {
        localStorage.setItem('soundcloud-desktop-auth-session-id', payload.sessionId);
      }
