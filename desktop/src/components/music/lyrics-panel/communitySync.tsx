...
import { getCurrentTime } from '../../../lib/audio';
...
import { shouldRenderPlainLyrics, type ResolvedLyricsData } from './lyricsData';
import { isPauseMarkerText } from './syncedLyrics';

export type CommunitySyncSource = 'genius' | 'soundcloud';
...
export function isCommunitySyncSource(source: LyricsSource | null | undefined): source is CommunitySyncSource {
...
export function createCommunitySyncLyricLine(text: string, time: number | null = null): CommunitySyncLine {
  return {
    kind: 'lyric',
    text,
    time,
...
export function findCommunitySyncNextStampedIndex(lines: CommunitySyncLine[], startIndex: number): number {
...
export function findCommunitySyncNextPendingIndex(lines: CommunitySyncLine[], startIndex: number): number {
  for (let index = Math.max(0, startIndex); index < lines.length; index += 1) {
